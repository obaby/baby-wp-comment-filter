...
### ⚙️ 管理功能
- **简单设置界面**: 直观的管理后台设置页面
- **错误消息自定义**: 可以自定义各种错误提示消息
- **统计信息**: 记录评论过滤统计信息
- **WordPress集成**: 与WordPress讨论设置完美集成

### 🔧 技术特性
- **简单架构**: 采用简单的面向对象架构，易于维护
- **性能优化**: 高效的过滤算法，不影响网站性能
- **兼容性**: 支持WordPress 3.0+版本
- **多语言**: 支持多语言环境
...
### Q: 如何添加更多自定义关键词？
A: 在插件设置页面的"自定义禁止关键词"文本框中，每行添加一个关键词即可。

### Q: 支持正则表达式吗？
A: 支持。在自定义关键词中使用 `/pattern/flags` 格式即可使用正则表达式。

### Q: 如何恢复默认设置？
A: 停用插件后重新激活即可恢复默认设置。

### Q: 插件与WordPress讨论设置的关系？
A: 插件可以同时使用WordPress后台"讨论设置"中的禁止关键词，两者是互补关系。
